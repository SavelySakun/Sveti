...
class ContactDeveloperVC: VCwithScrollView {
  ...
  @objc private func sendEmail() {
    if MFMailComposeViewController.canSendMail() {
      ...
      mailVC.setSubject("Sveti Feedback")
      present(mailVC, animated: true)
    } else {
      let alert = UIAlertController(title: "Error", message: "Sending an email is not supported. You need to set up mail", preferredStyle: .alert)
