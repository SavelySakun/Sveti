...
class TagCell: Cell {
    ...
    override func systemLayoutSizeFitting(_: CGSize, withHorizontalFittingPriority _: UILayoutPriority, verticalFittingPriority _: UILayoutPriority) -> CGSize {
        let heightOffset = (UIUtils.middleOffset * 2) + UIUtils.defaultOffset + 40
        let size = CGSize(width: Double(frame.width), height: Double(getCollectionViewHeight()) + Double(heightOffset))
        return size
    }
}
